...
function Client(options) {
	...
	this.endpoint =
		this.options.endpoint || "https://www.googleapis.com/youtube/v3";
}
...
Client.prototype._request = function (path, params, callback) {
	...
	request
		...
		.end(function (err, res) {
			if (err || !res.ok)
				return callback("An error ocurred in the request " + err);
...
Client.prototype.search = function (params) {
	return new Promise((res, rej) => {
		this._request("/search", params, function (err, data) {
			if (err) rej(err);
			res(data);
		});
	});
